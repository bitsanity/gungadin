...
public class ECIES
{
  ...
  public String encrypt( byte[] red ) throws Exception
  {
    ...
    byte[] d = curve.signECDSA( SHA256.hash(c), myPrivKey_ );
  ...
  {
    // parse message into R, c, d parts
    ...
    byte[] d = Base64.decode( parts[2] ); // signature of raw message by senders private key
    ...
    byte[] e = SHA256.hash( c );

    if ( curve.verifyECDSA(d, e, otherPubKey_) )
      return red;
